...
interface ResidentActionModalProps {
  isVisible: boolean;
  onClose: () => void;
  person: any;
}
...
const ResidentActionModal: React.FC<ResidentActionModalProps> = ({
  isVisible,
  onClose,
  person,
}) => {
  ...
  const availableActions = person.household_head
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      Alert.alert("Error", "Owner is house hold head");
      console.error(err);
    }
  };
  ...
  const handleAction = (actionId: string) => {
    switch (actionId) {
      case "view":
        navigation.navigate("UpdateOwnerForm", {
          owner: person,
        });
        break;
      case "delete":
        Alert.alert("Confirm Delete", "Are you sure you want to delete this?", [
          { text: "Cancel", style: "cancel" },
          {
            text: "Delete",
            style: "destructive",
            onPress: () => handleDelete(person.owner_id),
          },
        ]);
        break;
    }
