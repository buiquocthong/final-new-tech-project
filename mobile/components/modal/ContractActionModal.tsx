...
const ContractActionModal: React.FC<ContractActionModalProps> = ({
  isVisible,
  onClose,
  contract,
}) => {
  ...
  const handleDeleteContract = async (contract_id: string) => {
    ...
    try {
      ...
      await api.deleteContract(contract_id);
      Alert.alert("Success", "Delete invoice successfully", [
  ...
  const handleAction = (actionId: string) => {
    switch (actionId) {
      case "view":
        navigation.navigate("ContractDetailsForm", {
          contract: contract,
        });
        break;
      case "delete":
        ...
        <View style={styles.modalContent}>
          <View style={styles.header}>
            <View>
              ...
              <Text style={styles.blockName}>{contract.apartment.name}</Text>
            </View>
            <TouchableOpacity
...
export default ContractActionModal;
